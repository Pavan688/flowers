...
const inexpensiveFlowers = () => {
    ...
    // I will be using a for  of loop to iterate throuh flowers. 
    //i will be using consitional if statement to filter flowers less than 2 dollars and add the filtered flowers in a dif. variable. 
...
const zoneFlowers = () => {
    ...
    // Write your algorithm first, and then your code. Feel free to remove this comment
    // used a for of loop to iterate through flowers.
    //use a condoitional if satement to fliter usda zones with 3 and 8.
    //the filtered flowers will then be placed in a zoneFlower variable.
for (const flower of flowers) {
...
const orangeFlowers = () => {
    const filteredFlowers = []  // Do not modify
    // use a for of loop to iterate through function flowers.
    // use the consitional if statement to filter through flowers with orange color.
    // the filtered flowers will then be placed in orangeFlowers variable. 
for (const flower of flowers) {
...
for (const flower of flowers) { // first I started the for loop because we have do to the html for all the flowers
    flowersHTMLString += "<article>" // the top line is just a string
    flowersHTMLString += `<h1>${flower.type}</h1>`// the second i used siring interpaltion and added type of flowers in the with template literal.
    flowersHTMLString += "<h2>Colors</h2>" // this is just another string
    for (const color of flower.colors) { // i had to use a nested loop to iterate through the colors array.
        flowersHTMLString += `<section>${color}</section>`// used a string interpalation with template literal of colors.
    }
    flowersHTMLString += "<h2>USDA Zones</h2>"// another string 
    for (const zone of flower.usdaZones) { // another nested loop to iterate through the zone arrays of flowers.
        flowersHTMLString += `<div>${zone}</div>`// used string interpolation to add to the string with template literal of zones.
    }
    flowersHTMLString += "</article>" // this just added the end string.
}
